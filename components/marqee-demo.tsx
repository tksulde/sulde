...
import SimpleMarquee from './ui/marquee'
...
export default function SimpleMarqueeDemo() {
    ...
    return (
      ...
      >
        <motion.div className={textContainerClasses} variants={textVariants}>
          <h3 className="z-30 text-sm font-medium text-white sm:text-base md:text-lg">
            {album.title}
          </h3>
          <p className="z-30 text-xs text-neutral-300 sm:text-sm md:text-base">
            {album.artist}
          </p>
        </motion.div>
        <motion.img
  ...
  return (
    <div
      className="relative flex h-[60svh] w-full flex-col items-center justify-center overflow-x-hidden overflow-y-auto bg-black"
      ref={(node) => setContainer(node)}
    >
      <h1 className="font-calendas absolute top-1/4 text-center text-3xl text-white sm:text-5xl md:text-6xl">
        Weekly Mix
      </h1>
      ...
      {loading ? (
        ...
        <>
          <div
            className="absolute top-32 -left-3/4 flex h-1/2 w-[200%] flex-col items-center justify-center space-y-2 perspective-near sm:h-full sm:space-y-3 md:space-y-4"
            style={{
              transform:
